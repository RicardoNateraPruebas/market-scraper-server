...
app.post('/',async (req, res) => {
  ...
  if(req.body.Web=="Todas"){
    const results = await Promise.all([
      scraperController(browserInstance,mercadolivre[(req.body.search!=='') ? (req.body.Categorias+"Search"):req.body.Categorias],(req.body.search=='') ? false:true,"livre",cat[req.body.Categorias]),
      scraperController(browserInstance,buscape[(req.body.search!=='') ? (req.body.Categorias+"Search"):req.body.Categorias],(req.body.search=='') ? false:true,"buscape",cat[req.body.Categorias])
    ])
    
    response = results && results[0] ? results[0].concat(results[1]):[].concat(results[1])
